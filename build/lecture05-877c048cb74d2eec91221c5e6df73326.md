...
2. In your `sspa-sandbox` clone, create `./<name.surname>/` and copy the provided examples:  
   `cp -r /path/to/course/codes/lab05/{c++,python,vwce_2024.csv,fetch_vwce.sh} ./<name.surname>/`
...
1. In `./<name.surname>/c++`, add Doxygen-style comments to `main.cpp` (briefs, params, returns, etc.).
...
1. In `./<name.surname>/python`, add clear docstrings (Google or NumPy style) to functions in `main.py` covering parameters, returns, and examples.
2. In `./<name.surname>/`, run `sphinx-quickstart docs`. Enable `myst_parser`, `autodoc`, `autosummary`, `napoleon`, and `breathe` in `docs/conf.py`; set `autosummary_generate = True`.
